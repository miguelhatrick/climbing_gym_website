...
class WebMemberMembershipRequest(models.Model):
    """Medical certificates of each climbing gym member Web addition"""
    _inherit = ['climbing_gym.member_membership_request', 'portal.mixin']
    _name = 'climbing_gym.member_membership_request'

    def get_web_url(self):
        base_url = request.env['ir.config_parameter'].get_param('web.base.url')
        base_url += '/my/member_membership_requests/%s' % self.id
        return base_url

    def _compute_access_url(self):
        super(WebMemberMembershipRequest, self)._compute_access_url()
        for myobject in self:
            myobject.access_url = '/my/member_membership_requests/%s' % myobject.id
    ...
    def _get_share_url(self, redirect=False, signup_partner=False, pid=None):
        ...
        # return super(WebMemberMembershipRequest, self)._get_share_url(redirect, signup_partner, pid)

    @api.multi
    def preview_member_membership_request(self):
        self.ensure_one()
        return {
